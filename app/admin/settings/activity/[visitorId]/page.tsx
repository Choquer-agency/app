...
import { friendlyDate, friendlyDateTime } from "@/lib/date-format";
...
export default async function VisitorDetailPage({ params }: PageProps) {
  ...
  const recentVisits = sessions.filter((s) => new Date(s.session_start) > sevenDaysAgo).length;

  return (
    <div className="space-y-8">
      <div>
        <a href="/admin/settings/activity" className="text-sm text-accent hover:underline">&larr; Back to Activity</a>
        <h2 className="text-2xl font-bold mt-2">{visitor.visitor_name}</h2>
        <p className="text-sm text-muted mt-1">
          {visitor.client_slug} &middot; First seen{" "}
          {friendlyDate(visitor.created_at)}
          {visitor.device_types && (<> &middot; Devices: {(visitor.device_types as string[]).filter(Boolean).join(", ")}</>)}
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        {sessions.length === 0 ? (
          <p className="px-5 py-8 text-center text-sm text-muted">No sessions recorded yet.</p>
        ) : (
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="text-sm font-medium">
                        {friendlyDateTime(session.session_start)}
                      </p>
                      <p className="text-xs text-muted mt-1">Duration: {formatDuration(duration)}</p>
                    </div>
                    {session.clicked_cta && (
                      <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full font-medium">Clicked CTA</span>
                    )}
                  </div>
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {sections?.filter(Boolean).map((section) => (
                      <span key={section} className="text-xs bg-blue-50 text-blue-700 px-2 py-0.5 rounded-full">{section}</span>
                    ))}
                    {timeranges?.filter(Boolean).map((range) => (
                      <span key={range} className="text-xs bg-purple-50 text-purple-700 px-2 py-0.5 rounded-full">{range} view</span>
                    ))}
                    {months?.filter(Boolean).map((month) => (
                      <span key={month} className="text-xs bg-orange-50 text-orange-700 px-2 py-0.5 rounded-full">Opened {month}</span>
                    ))}
                  </div>
                </div>
              );
